...
type pollerStore interface {
	...
	ApplySourceEvents(context.Context, string, []ReminderIntent, time.Time) (ApplyResult, error)
}

func NewPoller(logger *slog.Logger, store pollerStore, client sourceClient, clock Clock, delaySource DelaySource, backoff ExponentialBackoff, source string, limit int, interval time.Duration) *Poller {
	return &Poller{
		logger:      logger,
		store:       store,
		client:      client,
...
func (p *Poller) Run(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			...
			p.logger.Warn("poll reminder source failed", "error_code", errorCode(err), "attempt", failures, "retry_in_ms", delay.Milliseconds())
...
func (p *Poller) PollOnce(ctx context.Context) (bool, error) {
	...
	page, err := p.client.ListReminderIntents(ctx, cursor, p.limit)
	if err != nil {
		return false, err
	}
	...
	for _, item := range page.Items {
		p.logger.Info(
			"reminder source event durably ingested",
